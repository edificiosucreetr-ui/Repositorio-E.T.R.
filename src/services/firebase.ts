...
// Claves de almacenamiento local seguro
const LOCAL_STORAGE_KEY = 'etrgc_projects';
const LOCAL_STORAGE_LOGO_KEY = 'etrgc_custom_logo';
...
export function loadLocalLogo(): string | null {
  try {
...
export function saveLocalLogo(logoUrl: string | null): void {
  try {
    if (logoUrl) {
      localStorage.setItem(LOCAL_STORAGE_LOGO_KEY, logoUrl);
    } else {
...
// Suscripción al logo institucional (Firebase RTDB + LocalStorage)
export function subscribeToLogo(
  onLogoChange: (logoUrl: string | null) => void
): () => void {
  // Primero emitir el valor local si existe
    ...
    const unsubscribe = onValue(
      ...
      (snapshot) => {
        ...
        if (cloudLogo !== undefined) {
          saveLocalLogo(cloudLogo);
          onLogoChange(cloudLogo);
        }
      },
      (error) => {
        console.warn("Uso de logo local (Firebase config no disponible):", error);
      }
    );
    return () => unsubscribe();
  } catch (e) {
